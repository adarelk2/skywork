...
if(!isset($_GET['do']) || empty($_GET['do']))
	...
	switch($_GET['do'])
	{
		...
		case "remove":
			...
			$query = $mysqli->query("SELECT * FROM `".PREFIX."_links` WHERE `id`='{$id}'");
			if ($query->num_rows == 0)
				die(' <meta http-equiv="Refresh" content="0; url=index.php?act=editpages&what=links">');
			
			//message
			$mysqli->query("DELETE FROM `".PREFIX."_links` WHERE `id`='{$id}'");
			refreshPage();
		break;
		default:
		...
		<div class="container">
			<div class="block">
				<div class="block-title">
					<h1>ניהול קישורים</h1>
				<a href="index.php?act=editpages&what=links&do=add">הוסף קישור חדש</a>
			...
			<span class="counter"></span>
				...
				<div class="block-inner">
				  ...
				  <thead>
					<tr>
						<th>#</th>
						<th class="col-md-5 col-xs-5">כותרת</th>
						<th class="col-md-4 col-xs-4">שם הקישור</th>
						<th class="col-md-3 col-xs-3">לינק</th>
				  ...
				  <tbody>
							<?php
								$counter = 1;
								$qLink = $mysqli->query("SELECT * FROM `".PREFIX."_links` ORDER BY `title`,`index`");
								while($Data = $qLink->fetch_assoc())
								{
							?>
							<tr>
