		...
		default:
			...
			if(!isset($_GET['do']) || empty($_GET['do']))
				...
				switch($_GET['do'])
				{
					...
					case "add":
						include "add-recommended.php";
					break;
					case "remove":
						if (!isset($_GET['id']) OR empty($_GET['id'])) $_GET['id'] = null;
						$id = secur($_GET['id'],"int");

						$query = $mysqli->query("SELECT * FROM `".PREFIX."_recommended` WHERE `id`='{$id}'");
						if ($query->num_rows == 0)
							die(' <meta http-equiv="Refresh" content="0; url=index.php?act=editpages">');
						
						//message
						$mysqli->query("DELETE FROM `".PREFIX."_recommended` WHERE `id`='{$id}'");
						refreshPage();
					break;
					default:
		...
		<div class="container">
			<div class="block">
				<div class="block-title">
					<h1>ניהול המלצות</h1>
				<a href="index.php?act=editpages&what=recommended&do=add">הוסף המלצה חדשה</a>
...
<span class="counter"></span>
  ...
  <thead>
    <tr>
		<th>#</th>
		<th class="col-md-5 col-xs-5">שם הממליץ</th>
		<th class="col-md-4 col-xs-4">שם החברה</th>
		<th class="col-md-3 col-xs-3">תוכן</th>
